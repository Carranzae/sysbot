...
class GroqKeyManager {
  ...
  public getStats() {
    ...
    const activeKeysCount = this.keys.filter(k => {
      if (k.isRateLimited && now > k.unbanAt) {
        k.isRateLimited = false
      }
      return !k.isRateLimited
    }).length

    const cooldownKeys = this.keys.map((k, index) => {
      const remainingSeconds = Math.max(0, Math.round((k.unbanAt - now) / 1000))
      return {
        keyIndex: index + 1,
        remainingSeconds,
        isRateLimited: k.isRateLimited && remainingSeconds > 0
      }
    }).filter(k => k.isRateLimited)

    return {
      totalKeys,
      activeKeysCount,
      cooldownKeysCount: cooldownKeys.length,
      cooldownKeys
    }
  }
...
export class AIService {
  async chat(message: string, history: any[] = [], providerId?: string, customerPhone?: string, isManagementMode = false, customSystemPrompt?: string) {
    ...
    try {
      ...
      let context = ''
      if (providerId && !customSystemPrompt) {
          if (isManagementMode) {
 ...
 ${lowStock.map(i => `• ${i.name}: Quedan ${i.stock}`).join('\n')}`
              ...
              if (products.length > 0) {
                  // Limitar a 5 productos y truncar nombres/precios para reducir tokens
                  context += `\nPRODUCTOS DISPONIBLES:\n${products.slice(0, 5).map((p: any) => `- ${p.name.slice(0, 40)}: S/ ${p.price}`).join('\n')}`
              }
      ...
      const systemPrompt = customSystemPrompt || (isManagementMode 
        ...
        try {
          const completion = await groq.chat.completions.create({
            messages: [
              ...
              ...history.slice(-4), // Max 4 mensajes de historial para no exceder TPM
              { role: 'user', content: message }
            ],
            model: 'llama-3.3-70b-versatile', // 12K TPM limit (vs 6K del 8b) — soporta prompts grandes del orquestador
            max_tokens: 400, // Respuestas cortas para respetar el límite de tokens
          }, { timeout: 15000 })
          ...
          const isPayloadTooLarge = err?.status === 413 // TPM exceeded para este request
          if (err?.status === 429 || err?.status === 401 || isTimeoutOrConnection || isServerError || isPayloadTooLarge) {
            const isAuthError = err?.status === 401
            const banTime = isAuthError ? 999999 : (isPayloadTooLarge ? 0.5 : 1) // 30s para 413, 1min para 429
            const reason = isAuthError ? 'Token Inválido (401)' : (isTimeoutOrConnection ? 'Timeout/Conexión' : isServerError ? `Error de servidor Groq (${err.status})` : isPayloadTooLarge ? 'Prompt muy grande (413-TPM)' : `Rate Limit (429): ${err?.message?.substring(0, 100)}`)
            ...
            continue // Probar la siguiente llave en el loop
          }
          // Para cualquier otro error, logear con detalles completos y continuar con siguiente llave
          logger.warn(`⚠️ [AI-CHAT] Error inesperado en llave #${keyIndex}: ${err?.message} (status: ${err?.status}, name: ${err?.name}). Intentando siguiente llave...`)
          continue
        }
      }
      ...
      logger.error('Error en AI Chat:', { error: e?.message, status: e?.status, name: e?.name, stack: e?.stack?.slice(0, 300) })
      return { text: 'Lo siento, hubo un error procesando tu solicitud. Por favor, intenta de nuevo más tarde.' }
    }
  }

  async chatStream(message: string, history: any[] = [], userId?: string, isManagementMode = false, cartItems: any[] = [], catalogContext: string = 'nacional') {
    try {
      ...
      if (isManagementMode) {
        ...
        const looksLikeProductSearch = /producto|vendes|tienes|precio|oferta|comprar|buscar|quiero|nike|adidas|iphone|zapatilla|reloj|marca|modelo/i.test(message)
        if (looksLikeProductSearch) {
            const products = await this.searchProductsForAI(message)
            if (products.length > 0) {
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          ...
          const isTimeoutOrConnection = err?.name === 'APITimeoutError' || err?.name === 'APIConnectionError' || err?.code === 'ETIMEDOUT'
          if (err?.status === 429 || err?.status === 401 || isTimeoutOrConnection) {
            const isAuthError = err?.status === 401
            const banTime = isAuthError ? 999999 : 1
            const reason = isAuthError ? 'Token Inválido (401)' : (isTimeoutOrConnection ? 'Timeout/Conexión' : 'Rate Limit (429)')
            keyManager.reportRateLimit(keyIndex, banTime, reason)
  ...
  async searchProductsForAI(query: string) {
    try {
      // Búsqueda exacta (Marca + Modelo + Atributo)
      const words = query.split(/\s+/).filter(w => w.length > 2);
      let sql = `SELECT id, name, price, description, stock, category FROM products WHERE stock > 0 AND (`;
      let params = [];
      
      if (words.length > 0) {
          const conditions = words.map((w, i) => {
              params.push(`%${w}%`);
              return `(name ILIKE $${i+1} OR description ILIKE $${i+1} OR category ILIKE $${i+1})`;
          });
          sql += conditions.join(' AND ');
      } else {
          sql += `name ILIKE $1 OR description ILIKE $1 OR category ILIKE $1`;
          params.push(`%${query}%`);
      }
      
      sql += `) ORDER BY stock DESC LIMIT 5`;

      const { rows } = await db.query(sql, params)
      if (rows.length > 0) return rows;

      // Fallback a una búsqueda más flexible
